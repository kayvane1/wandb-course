...
def parse_args():
    ...
    argparser = argparse.ArgumentParser(description='Prepare Data for Complaints Allocation')
    argparser.add_argument('--dataset', type=str, default=default_cfg.dataset, help='Name of the Dataset on the Huggingface Hub')
    argparser.add_argument('--text_column', type=str, default=default_cfg.text_column, help='Column in the dataset whichc ontains the text data / complaint background and narrative')
...
def log_raw_data(cfg):
    ...
    with wandb.init(project=cfg.PROJECT_NAME, job_type=cfg.RAW_DATA_JOB_TYPE, config=dict(cfg)) as run:
        ...
        text_dataset = load_dataset(default_cfg.dataset, split=split_percentage, ignore_verifications=True)
...
def process_and_log_data(cfg):
    ...
    with wandb.init(project=cfg.PROJECT_NAME, job_type=cfg.DATA_PROCESSING_JOB_TYPE, config=dict(cfg)) as run:
...
def split_and_log_data(cfg):
    ...
    with wandb.init(project=cfg.PROJECT_NAME, job_type=cfg.SPLIT_DATA_JOB_TYPE, config=dict(cfg)) as run:
